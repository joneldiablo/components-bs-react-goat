...
export interface RangeFieldstate extends FieldState {
  min?: number | string;
  max?: number | string;
}
...
> {
  ...
  onUpdate(updates: Record<string, any>) {
    ...
    eventHandler.dispatch(`update.${this.props.name}-fromControl`, {
      value: from,
    });
  ...
  onValuesChange(data: any) {
    ...
    this.setState(newState, () => {
      const state = this.state as RangeFieldstate;
      this.onChange({
        target: {
  ...
  mutations(name: string, section: Record<string, any>) {
    switch (name) {
      ...
      case `${this.props.name}-toControl`: {
        ...
        const controlClasses: any = new Set(
          [this.props.controlClasses]
            .flat()
            .map((c) => v.split(" "))
            .flat()
        );
        ...
        return Object.assign(
          ...
          name.endsWith("fromControl") ? this.props.from : this.props.to,
          {
            ...
            [minMax]: (this.state as any)[minMax],
      ...
      case `${this.props.name}-divisor`: {
        ...
        (classes as any)[addDelete]("border-0");
        return {
          classes: Array.from(classes).flat().join(" "),
          style: {
            backgroundColor: this.props.disabled
              ? "var(--bs-secondary-bg)"
              : "var(--bs-transparent-bg)",
          },
