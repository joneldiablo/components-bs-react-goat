...
**Origin:** `js/components.js` in `dbl-components`.
**Destination:** `js/components.js` in `components-bs-react-goat`.
**Existing TS:** Yes, already exists in `react-goat` at `components.ts`.
