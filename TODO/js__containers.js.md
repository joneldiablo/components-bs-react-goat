...
**Origin:** `js/containers.js` in `dbl-components`.
**Destination:** `js/containers.js` in `components-bs-react-goat`.
**Existing TS:** Yes, already exists in `react-goat` at `containers.ts`.
